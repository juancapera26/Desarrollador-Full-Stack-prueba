...
export class CartPage implements OnInit {
  ...
  async removeItem(item: CartItem): Promise<void> {
    ...
    await this.runItemAction(item.product.id, async () => {
      this.cart = await this.cartService.removeItem(this.currentUser!.id, item.product.id);
  ...
  private async runItemAction(productId: number, action: () => Promise<void>): Promise<void> {
    this.busyProductIds.add(productId);
    this.actionMessage = '';
    try {
      await action();
    } catch {
      this.actionMessage = 'No pudimos actualizar el carrito. Intenta nuevamente.';
    } finally {
